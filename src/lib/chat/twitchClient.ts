...
export function connectTwitch(
  targets: { streamer: StreamerId; login: string }[],
  onMessage: (msg: ChatMessage) => void
): () => void {
  if (!targets.length) return () => {};

  const byChannel = new Map<string, StreamerId>();
  for (const t of targets) byChannel.set(t.login.toLowerCase(), t.streamer);
  ...
  function open() {
    ...
    ws.onopen = () => {
      ...
      for (const t of targets) ws!.send(`JOIN #${t.login.toLowerCase()}`);
    };
